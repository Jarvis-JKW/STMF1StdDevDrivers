/**
  ...
  * 模拟IIC协议不受限于IO口, 可新建结构体以使用, 需要初始化IO为推挽输出模式
  * IIC_Touch: LCD GT917S(外接)
  ******************************************************************************
  */
...
/**
  ...
  * @retval None
  */
void IIC_Response(IIC_TypeDef* IICx, BitAction ACK_State)
{
    GPIO_ResetBits(IICx->SCL_Port, IICx->SCL_Pin);
...
/**
  ...
  * @retval uint8_t data
  */
uint8_t IIC_Read(IIC_TypeDef* IICx ,uint8_t slaveAddr, uint16_t regAddr)
{
    uint8_t temp = 0;
    ...
    /* 发送从设备地址+写命令 */
    IIC_SendData(IICx, slaveAddr|0);
    if(IIC_Wait_ACK(IICx)!=SUCCESS)
        goto timeout;

    /* 写寄存器地址，双字节寻址 */
    IIC_SendData(IICx, regAddr>>8);
    if(IIC_Wait_ACK(IICx)!=SUCCESS)
        goto timeout;

    IIC_SendData(IICx, regAddr&0xff);
    if(IIC_Wait_ACK(IICx)!=SUCCESS)
        goto timeout;

    IIC_GenerateSTART(IICx);
    /* 发送从设备地址+读命令 */
    IIC_SendData(IICx, slaveAddr|1);
    if(IIC_Wait_ACK(IICx)!=SUCCESS)
        goto timeout;
...
/**
  ...
  * @retval 0:成功 | 1: 失败
  */
uint8_t IIC_Write(IIC_TypeDef* IICx ,uint8_t slaveAddr, uint16_t regAddr, uint8_t data)
{
    IIC_GenerateSTART(IICx);
...
/**
  ...
  */
uint8_t IIC_ReadBytes(IIC_TypeDef* IICx, uint8_t slaveAddr, uint16_t regAddr, uint8_t *data, uint8_t len)
{
    IIC_GenerateSTART(IICx);
...
/**
  ...
  */
//uint8_t IIC_WriteBytes(IIC_TypeDef* I2Cx, uint8_t slaveAddr, uint16_t regAddr, uint8_t *data, uint8_t len)
//{
//    while(len--)
//    {
//        if(I2C_WriteByte(I2Cx, slaveAddr, regAddr++, *data++))
//            return 1;
//    }
//    return 0;
//}

uint8_t IIC_WriteBytes(IIC_TypeDef* IICx, uint8_t slaveAddr, uint16_t regAddr, uint8_t *data, uint8_t len)
{
    IIC_GenerateSTART(IICx);
