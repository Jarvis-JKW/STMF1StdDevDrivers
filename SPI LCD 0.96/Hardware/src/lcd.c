...
{
    LCD_Fill(0, 0, LCD_Configuration.width, LCD_Configuration.height, color);
...
/**
  ...
  * @retval None
  */
void LCD_Puts(uint16_t x, uint16_t y, char *p, uint8_t sizeY, uint8_t mode)
{
    while(*p != '\0')
    {
        LCD_Putchar(x, y, *p++, sizeY, mode);
        x += sizeY / 2; 
    }
}
...
/**
  ...
  * @retval None
  */
void LCD_Putzh(uint16_t x, uint16_t y, char *p, uint8_t sizeY, uint8_t mode)
{
    uint8_t i, j, m = 0;
    uint16_t k, num;
    uint16_t x0 = x, TypefaceNum = (sizeY/8+((sizeY%8)?1:0))*sizeY;
    ...
    while(*p != '\0')
    {
        for(k = 0; k<num; k++)
        {
            ...
            {     
                LCD_SetRefreshArea(x, y, x+sizeY-1, y+sizeY-1);
                for(i = 0; i < TypefaceNum; i++)
                {
                    for(j = 0; j < 8; j++)
                    {    
                        ...
                        else /* 叠加方式 */
                        {
                            if(((tfont12[k].Msk[i]&(0x01<<j))&&(sizeY==12))||((tfont16[k].Msk[i]&(0x01<<j))&&(sizeY==16))||
                               ((tfont24[k].Msk[i]&(0x01<<j))&&(sizeY==24))||((tfont32[k].Msk[i]&(0x01<<j))&&(sizeY==32)))
                                LCD_DrawPoint(x, y, LCD_Configuration.penColor);//画一个点
                            x++;
                            if((x-x0)==sizeY)
                            {
                                x = x0;
